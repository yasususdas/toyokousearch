...
function updatePinPosition(pin, cellValue, offset = 0) {
    ...
    if (cellValue === "Shokuin_Shitsu") {
        ...
        pin.style.left = "36.2%";
    } else if (cellValue === "Seibutsu_Junbi_Shitsu") {
        pin.style.top = offset === 0 ? "81.9%" : `calc(81.9% - ${offset}%)`;
        pin.style.left = "28%";
                ...
                if (pins[pinKey] && pins[pinKey].element) {
                    if (status === "exit") {
                        ...
                        if (offsetCellValues.includes(location)) {
                            ...
                            pins[pinKey].element.style.zIndex = 100 - offset;
                        } else {
                            updatePinPosition(pins[pinKey].element, location);
                            pins[pinKey].element.style.zIndex = 100;
                        }
                    }
                }
            });
